...
public class JExternalFrame extends JWindow implements WindowConstants {
  ...
  public static int RELATIVECHANGEBEFORERESIZE = 4;
  protected JDesktopPane framePane;
 ...
 * @created   27 avril 2002
 */
protected class MiniDesktopManager
    ...
    public void setBoundsForFrame(
        ...
        int newHeight) {
        Dimension d = getSize();
        boolean didResize = ((d.width != newWidth) || (d.height != newHeight));
...
public void mouseDragged(java.awt.event.MouseEvent e) {
    ...
    int heightAdjustement = 0;
    int changedWidth = getSize().width;
    int changedHeight = getSize().height;
    switch (direction) {

        case WEST :
            widthAdjustment = -newMouseLocation.x;
            xAdjustment = newMouseLocation.x;
            break;
        case NORTH_WEST :
            widthAdjustment = -newMouseLocation.x;
            heightAdjustement = -newMouseLocation.y;
            xAdjustment = newMouseLocation.x;
            yAdjustment = newMouseLocation.y;
            break;
        case NORTH :
            heightAdjustement = -newMouseLocation.y;
            yAdjustment = newMouseLocation.y;
            break;
        case NORTH_EAST :
            newMouseLocation.x -= changedWidth;
            widthAdjustment = newMouseLocation.x;
            heightAdjustement = -newMouseLocation.y;
            yAdjustment = newMouseLocation.y;
            break;
        case EAST :
            widthAdjustment = newMouseLocation.x - changedWidth;
            break;
        case SOUTH_EAST :
            widthAdjustment = newMouseLocation.x - changedWidth;
            heightAdjustement = newMouseLocation.y - changedHeight;
            break;
        case SOUTH :
            heightAdjustement = newMouseLocation.y - changedHeight;
            ...
            break;
        case SOUTH_WEST :
            newMouseLocation.y -= changedHeight;
            widthAdjustment = -newMouseLocation.x;
            heightAdjustement = newMouseLocation.y;
            xAdjustment = newMouseLocation.x;
            break;
    ...
    /*System.out.println("Mouse location:" + newMouseLocation);
    System.out.println("Screen location:" + getLocation().toString());
    System.out.println("Current width:"+changedWidth);
    System.out.println("Current height:"+changedHeight);
    System.out.println("X adj:"+xAdjustment);
    System.out.println("Y adj:"+yAdjustment);
    System.out.println("Width adj:"+widthAdjustment);
    System.out.println("Height adj:"+heightAdjustement);*/

    changedWidth += widthAdjustment;
    changedHeight += heightAdjustement;
    if (Math.abs(widthAdjustment + heightAdjustement) > JExternalFrame.RELATIVECHANGEBEFORERESIZE) {
        setBoundsForFrame(
            (JInternalFrame) e.getSource(),
            xAdjustment,
            yAdjustment,
            changedWidth,
            changedHeight);
...
public JExternalFrame(
    ...
    MiniDesktopManager miniDesk = new MiniDesktopManager();
    realFrame.addMouseMotionListener(miniDesk);
    framePane.setDesktopManager(miniDesk);
    c.add(framePane);
   ...
   * closable, maximizable, iconifiable
   */
  public JExternalFrame(boolean NoArg) {
    super();
  }
  //Ends constructor JExternalFrame
   ...
   */
  public void forwardWindowEvent(int WindowEventID) {
   ...
   * @return   the Container where all your stuff should go.
   */
  public Container getJWindowContentPane() {
    return super.getContentPane();
  }
/**
 * Insert the method's description here.
 * Creation date: (5/21/2003 11:53:54 AM)
 * @return javax.swing.JInternalFrame
 */
public javax.swing.JInternalFrame getRealFrame() {
	return realFrame;
}
